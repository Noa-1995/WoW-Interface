...
--	piece:SetVertTile(setupInfo.tileVertical)
	piece:SetSubTexCoords(left, right, top, bottom)
end
...
local layouts =
{
	...
	{
		TopLeftCorner =	{ layer = "OVERLAY", atlas = "UI-Frame-PortraitMetal-CornerTopLeft-2x", x = -13, y = 16, },
...
-- NINE SLICE PANEL MIXIN
 NineSlicePanelTableMixin = {}

function NineSlicePanelTableMixin:GetFrameLayoutType()
	return self:GetAttribute("layoutType") or self:GetParent():GetAttribute("layoutType")
end

function NineSlicePanelTableMixin:SetParentFrameLevel()
...
function NineSlicePanelTableMixin:OnLoad()
	local layout = NineSliceUtils.GetLayout(self:GetFrameLayoutType())
	if layout then
		NineSliceUtils.ApplyLayout(self, layout)
