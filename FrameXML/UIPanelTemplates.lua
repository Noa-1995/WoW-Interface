...
local function PanelTemplates_ResolveNumTabs(frame)
	local numTabs = tonumber(frame and frame.numTabs);
	if numTabs then
		return numTabs;
	end
	...
	local count = 0;
	while _G[frameName.."Tab"..(count + 1)] do
		count = count + 1;
	end
	frame.numTabs = count;
	return count;
...
function PanelTemplates_UpdateTabs(frame)
	if ( not frame or not frame.selectedTab ) then
		return;
	end
	local numTabs = PanelTemplates_ResolveNumTabs(frame);
	if numTabs <= 0 then
		return;
	end
	for i = 1, numTabs do
