...
function GuildControlPopupframe_Update(loadPendingTabPermissions, skipCheckboxUpdate)
	...
	if ( not numRanks or numRanks < 1 ) then
		return;
	end
	local rankID = UIDropDownMenu_GetSelectedID(GuildControlPopupFrameDropDown) or 1;
	if ( rankID < 1 or rankID > numRanks ) then
		rankID = 1;
	end
	local rankName = GuildControlGetRankName(rankID);
	if ( not rankName ) then
		return;
	end
	UIDropDownMenu_SetSelectedID(GuildControlPopupFrameDropDown, rankID);
	GuildControlPopupFrameEditBox:SetText(rankName);
...
-- ==========================================================================
-- Noa Social shell: una sola ventana NineSlice para Amigos/Quién/Hermandad/
-- Canales/Raid. Conserva todos los controladores nativos de 3.3.5.
-- ==========================================================================
local function FriendsFrameNoa_HideLegacyArt()
	local names = {
...
local function FriendsFrameNoa_CreateInset(parent, name)
	local inset = CreateFrame("Frame", name, parent, "InsetFrameTemplateNoBackground")
	inset:SetFrameLevel(parent:GetFrameLevel() + 3)
	inset:EnableMouse(false)
	return inset
end

local FRIENDS_NOA_COMPACT_WIDTH = 465
local FRIENDS_NOA_GUILD_WIDTH = 720
local FRIENDS_NOA_HEIGHT = 560
local FRIENDS_NOA_TAB_TEXTURE = "Interface\\FrameXML\\Textures\\uiframe-tab"

local function FriendsFrameNoa_UpdateLayout()
	local frame = FriendsFrame
	if not frame or not frame.NoaSocialInitialized then return end

	local guildTab = frame.selectedTab == 3
	local wantedWidth = guildTab and FRIENDS_NOA_GUILD_WIDTH or FRIENDS_NOA_COMPACT_WIDTH
	if frame:GetWidth() ~= wantedWidth then
		frame:SetWidth(wantedWidth)
	end

	if frame.NoaGuildDetailFill then
		if guildTab then
			frame.NoaGuildDetailFill:Show()
			frame.NoaGuildDetailInset:Show()
		else
			frame.NoaGuildDetailFill:Hide()
			frame.NoaGuildDetailInset:Hide()
			if GuildMemberDetailFrame then GuildMemberDetailFrame:Hide() end
		end
	end
end

local function FriendsFrameNoa_SetTabPiece(texture, left, right, top, bottom, width, height)
	if not texture then return end
	texture:SetTexture(FRIENDS_NOA_TAB_TEXTURE)
	texture:SetTexCoord(left, right, top, bottom)
	if width then texture:SetWidth(width) end
	texture:SetHeight(height)
end

local function FriendsFrameNoa_ReskinTab(tab)
	if not tab or tab.NoaReskinned then return end
	local name = tab:GetName()
	FriendsFrameNoa_SetTabPiece(_G[name.."Left"], 0.015625, 0.562500, 0.816406, 0.957031, 35, 36)
	FriendsFrameNoa_SetTabPiece(_G[name.."Right"], 0.015625, 0.593750, 0.667969, 0.808594, 37, 36)
	FriendsFrameNoa_SetTabPiece(_G[name.."Middle"], 0.000000, 0.015625, 0.175781, 0.316406, nil, 36)
	FriendsFrameNoa_SetTabPiece(_G[name.."LeftDisabled"], 0.015625, 0.562500, 0.496094, 0.660156, 35, 42)
	FriendsFrameNoa_SetTabPiece(_G[name.."RightDisabled"], 0.015625, 0.593750, 0.324219, 0.488281, 37, 42)
	FriendsFrameNoa_SetTabPiece(_G[name.."MiddleDisabled"], 0.000000, 0.015625, 0.003906, 0.167969, nil, 42)

	local left, right, middle = _G[name.."Left"], _G[name.."Right"], _G[name.."Middle"]
	local leftD, rightD, middleD = _G[name.."LeftDisabled"], _G[name.."RightDisabled"], _G[name.."MiddleDisabled"]
	...
	if middle and left and right then
		middle:ClearAllPoints(); middle:SetPoint("TOPLEFT", left, "TOPRIGHT"); middle:SetPoint("TOPRIGHT", right, "TOPLEFT"); middle:SetHorizTile(true)
	end
	if leftD then leftD:ClearAllPoints(); leftD:SetPoint("TOPLEFT", tab, "TOPLEFT", -1, 0) end
	if rightD then rightD:ClearAllPoints(); rightD:SetPoint("TOPRIGHT", tab, "TOPRIGHT", 8, 0) end
	if middleD and leftD and rightD then
		middleD:ClearAllPoints(); middleD:SetPoint("TOPLEFT", leftD, "TOPRIGHT"); middleD:SetPoint("TOPRIGHT", rightD, "TOPLEFT"); middleD:SetHorizTile(true)
	end

	local oldHighlight = tab:GetHighlightTexture()
	if oldHighlight then oldHighlight:SetTexture(nil); oldHighlight:Hide() end
	local highlight = tab:CreateTexture(nil, "HIGHLIGHT")
	highlight:SetTexture("Interface\\QuestFrame\\UI-QuestTitleHighlight")
	highlight:SetBlendMode("ADD")
	highlight:SetAlpha(0.45)
	highlight:SetPoint("TOPLEFT", tab, "TOPLEFT", 4, -4)
	highlight:SetPoint("BOTTOMRIGHT", tab, "BOTTOMRIGHT", -4, 5)
	tab.NoaReskinned = true
end

local function FriendsFrameNoa_Initialize(frame)
	if not frame or frame.NoaSocialInitialized then return end
	frame.NoaSocialInitialized = true
	frame:SetSize(FRIENDS_NOA_COMPACT_WIDTH, FRIENDS_NOA_HEIGHT)
	frame:SetClampedToScreen(true)

	FriendsFrameNoa_HideLegacyArt()

	local background = frame:CreateTexture(nil, "BACKGROUND")
	background:SetPoint("TOPLEFT", frame, "TOPLEFT", 6, -22)
	background:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -6, 6)
	background:SetTexture("Interface\\FrameGeneral\\UI-Background-Rock")
	background:SetVertexColor(0.16, 0.15, 0.14, 0.90)
	frame.NoaBackground = background

	local leftFill = frame:CreateTexture(nil, "BACKGROUND")
	leftFill:SetPoint("TOPLEFT", frame, "TOPLEFT", 12, -56)
	leftFill:SetPoint("BOTTOMRIGHT", frame, "BOTTOMLEFT", 453, 36)
	leftFill:SetTexture("Interface\\Tooltips\\UI-Tooltip-Background")
	leftFill:SetVertexColor(0.025, 0.025, 0.025, 0.70)
	frame.NoaListFill = leftFill

	local listInset = FriendsFrameNoa_CreateInset(frame, "FriendsFrameNoaListInset")
	listInset:SetPoint("TOPLEFT", leftFill, "TOPLEFT", 0, 0)
	listInset:SetPoint("BOTTOMRIGHT", leftFill, "BOTTOMRIGHT", 0, 0)
	frame.NoaListInset = listInset

	-- La pestaña de Hermandad conserva una columna de lista compacta. Al
	-- ensanchar el PortraitFrame, esta columna no se estira sobre el detalle.
	if GuildFrame then
		GuildFrame:ClearAllPoints()
		GuildFrame:SetPoint("TOPLEFT", frame, "TOPLEFT", 12, -56)
		GuildFrame:SetPoint("BOTTOMRIGHT", frame, "BOTTOMLEFT", 453, 36)
	end

	local detailFill = frame:CreateTexture(nil, "BACKGROUND")
	detailFill:SetPoint("TOPLEFT", frame, "TOPLEFT", 458, -56)
	detailFill:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -12, 36)
	detailFill:SetTexture("Interface\\Tooltips\\UI-Tooltip-Background")
	detailFill:SetVertexColor(0.035, 0.03, 0.025, 0.82)
	detailFill:Hide()

	local detailInset = FriendsFrameNoa_CreateInset(frame, "FriendsFrameNoaGuildDetailInset")
	detailInset:SetPoint("TOPLEFT", detailFill, "TOPLEFT")
	detailInset:SetPoint("BOTTOMRIGHT", detailFill, "BOTTOMRIGHT")
	detailInset:Hide()
	frame.NoaGuildDetailFill = detailFill
	frame.NoaGuildDetailInset = detailInset

	if GuildMemberDetailFrame then
		GuildMemberDetailFrame:SetParent(frame)
		GuildMemberDetailFrame:SetBackdrop(nil)
		GuildMemberDetailFrame:ClearAllPoints()
		GuildMemberDetailFrame:SetPoint("TOPLEFT", frame, "TOPLEFT", 468, -70)
		GuildMemberDetailFrame:SetSize(232, 390)
		GuildMemberDetailFrame:SetFrameStrata(frame:GetFrameStrata())
		GuildMemberDetailFrame:SetFrameLevel(frame:GetFrameLevel() + 8)
		if GuildMemberDetailCloseButton then GuildMemberDetailCloseButton:Hide() end
		if GuildMemberDetailCorner then GuildMemberDetailCorner:Hide() end
	end

	if FriendsFramePortrait then
		FriendsFramePortrait:SetTexture("Interface\\FriendsFrame\\FriendsFrameScrollIcon")
	end

	if SidebarScrollFrame_Initialize then
		local scrollFrames = {
			FriendsFrameFriendsScrollFrame, FriendsFrameIgnoreScrollFrame,
			WhoListScrollFrame, GuildListScrollFrame, GuildInfoFrameScrollFrame,
			GuildEventLogScrollFrame,
		}
		for _, scrollFrame in ipairs(scrollFrames) do
			if scrollFrame then
				SidebarScrollFrame_Initialize(scrollFrame, { minThumbHeight = 20, width = 8 })
			end
		end
	end
	...
	for index = 1, 5 do
		...
		if tab then
			...
			local textWidth = text and text:GetStringWidth() or 50
			tab:SetWidth(math.max(70, textWidth + 24))
			tab:ClearAllPoints()
			if previous then
	...
	FriendsFrameNoa_UpdateLayout()
end

local FriendsFrame_OnLoad_Native = FriendsFrame_OnLoad
function FriendsFrame_OnLoad(self)
	FriendsFrame_OnLoad_Native(self)
	FriendsFrameNoa_Initialize(self)
end

local FriendsFrame_Update_Native = FriendsFrame_Update
function FriendsFrame_Update(...)
	FriendsFrame_Update_Native(...)
	FriendsFrameNoa_HideLegacyArt()
	FriendsFrameNoa_UpdateLayout()
end
