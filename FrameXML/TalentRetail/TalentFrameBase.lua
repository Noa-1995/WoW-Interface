...
function TalentFrame_Update(TalentFrame)
	...
	if PlayerTalentFrame and (PlayerTalentFrame.resetTalentsForPreview or PlayerTalentFrame.learnPreviewTalentsAnimation or PlayerTalentFrame.previewState) then
		...
		if ( i <= numTalents ) then
			...
			if ( name and tier <= MAX_NUM_TALENT_TIERS) then
				...
				if (goldBorder and button.GoldBorder) then
					TalentRetail_SetShown(button.GoldBorder, selectedTalentGroup < 3)
					TalentRetail_SetShown(button.EtherealGoldBorder, selectedTalentGroup > 2)
					button.Slot:Hide();
				...
				if ( prereqsSet and ((preview and meetsPreviewPrereq) or (not preview and meetsPrereq)) ) then
					...
					if ( displayRank < maxRank and not PlayerTalentFrame.resetTalentsForPreview and not PlayerTalentFrame.learnPreviewTalentsAnimation ) then
